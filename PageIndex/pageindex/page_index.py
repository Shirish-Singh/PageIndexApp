...
def generate_toc_continue(toc_content, part, model="gpt-4o-2024-11-20"):
    ...
    return _ensure_list_of_dicts(result)
...
def _ensure_list_of_dicts(data):
    """Ensure the data is a list of dictionaries with required fields."""
    if data is None:
        return []

    # If it's already a list, validate items
    ...
    # If it's a dict, try to extract list from known keys
    if isinstance(data, dict):
        for key in ['table_of_contents', 'structure', 'data', 'items', 'sections']:
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    ...
    # If TOC generation failed, create a basic structure
    if not toc_with_page_number:
        logger.info('TOC generation returned empty, creating basic structure')
...
def check_toc(page_list, opt=None):
    ...
    if len(toc_page_list) == 0:
        ...
        if toc_json.get('page_index_given_in_toc', 'no') == 'yes':
            print('index found')
            return {'toc_content': toc_json.get('toc_content', ''), 'toc_page_list': toc_page_list, 'page_index_given_in_toc': 'yes'}
        else:
            current_start_index = toc_page_list[-1] + 1

            while (toc_json.get('page_index_given_in_toc', 'no') == 'no' and
                ...
                if additional_toc_json.get('page_index_given_in_toc', 'no') == 'yes':
                    print('index found')
                    return {'toc_content': additional_toc_json.get('toc_content', ''), 'toc_page_list': additional_toc_pages, 'page_index_given_in_toc': 'yes'}

                else:
                    current_start_index = additional_toc_pages[-1] + 1
            print('index not found')
            return {'toc_content': toc_json.get('toc_content', ''), 'toc_page_list': toc_page_list, 'page_index_given_in_toc': 'no'}
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        if mode == 'process_toc_with_page_numbers':
            ...
            # Fallback: create a basic structure with the whole document as one section
            logger.info("All processing modes failed, creating basic document structure")
            return [{
                'structure': '1',
                'title': 'Document Content',
                'physical_index': start_index,
